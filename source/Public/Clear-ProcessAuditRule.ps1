function Clear-ProcessAuditRule {
    <#
    .SYNOPSIS
        Clears selected explicit audit rules from pinned live processes.
    .DESCRIPTION
        Removes explicit process SACL entries for selected accounts, or every
        explicit audit ACE when Account is omitted, on the pinned instance.
    ...
    .EXAMPLE
        Clear-ProcessAuditRule -ProcessId $PID -Account Everyone -WhatIf

        Previews removing explicit Everyone audit rules from the current process.
    .INPUTS
        System.Int32
    ...
    process {
        ...
        foreach ($target in $targets) {
            if ($PSCmdlet.ShouldProcess($target.CanonicalTarget, 'Clear explicit process audit rules')) {
                $removed = if ($PassThru) {
                    $getRuleParameters = @{
                        Target           = $target
                        RuleType         = 'Audit'
