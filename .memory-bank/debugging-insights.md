...
## A PowerShell class attribute resolves only under its full class name

`[WindowsAccessRightsTransform([FileSystemRights])]` on a public parameter made
every call from outside the module fail with `Cannot find the type for custom
attribute 'WindowsAccessRightsTransform'`, while the same attribute worked on a
private function called inside the module. A three-function probe module isolated
it: the engine's implicit `Attribute` suffix search does not reach a PowerShell
class type, so `[FooTransform(...)]` fails and `[FooTransformAttribute(...)]`
works. A class that does not carry the suffix at all also works. Both editions
behave the same. Name a class-based attribute exactly as it is referenced.

## An argument transformation is bypassed for some argument shapes

The same transformation attribute converts a raw access mask for a decimal
literal, a variable, a parenthesized expression and a quoted hexadecimal string,
but not for a bare hexadecimal literal in argument mode and not for the result of
`Get-Variable -ValueOnly`. In those two shapes the transform runs, returns the
enum, and the engine still re-converts the original value and rejects it. The
practical rule for callers and tests is to pass a raw mask as a decimal number, a
quoted hexadecimal string, or through a variable.

## An access control list unrolls when it leaves an if expression

`$acl = if ($isAudit) { $descriptor.SystemAcl } else { $descriptor.DiscretionaryAcl }`
assigns the ACEs, not the list, because a `RawAcl` is enumerable and a value
flowing out of a statement is unrolled. The symptom was
`[CommonAce] does not contain a method named 'RemoveAce'`. Assign inside each
branch instead, which is what `Invoke-WindowsAclRuleMutation` already did.

## Windows maps generic rights on an effective entry when the descriptor is written

An access control entry carrying `GENERIC_ALL` keeps that mask only while it is
inherit-only. Written as an effective entry, it is stored as `0x1F01FF`
(`FullControl`) after the file system generic mapping. `icacls` goes further and
applies the mapping when it renders any mask, so it prints `F` or `M` for an
entry whose stored mask still carries the generic bits. A test that expects
`icacls` to print `GA` on an NTFS object is asserting something Windows never
does.

## A noncanonical DACL is stored exactly as written

Writing an allow entry before a deny entry through `SetSecurityInfo` neither
reorders the list nor sets `SE_DACL_PROTECTED`. The item keeps the noncanonical
order, `AccessRulesCanonical` reports false, `AccessRulesProtected` stays false,
and `icacls /verify` reports `Ace entries not in canonical order`.

## A self-referential junction breaks TestDrive teardown

A junction inside `$TestDrive` that points at its own parent makes Pester's
cleanup recurse until the path exceeds every limit, and the whole container
fails after the tests have already passed. Keep a link fixture outside
`$TestDrive`, track every link, and delete each link before the tree.
